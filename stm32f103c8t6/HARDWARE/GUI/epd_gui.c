...
void Image_Init(uint8_t *image, unsigned short int width, unsigned short int height, unsigned short int Rotate, unsigned short int Color)
{
    ...
    Image.color = Color;
    Image.scale = 7;     
    Image.byte_w = (width % 2 == 0)? (width / 2 ): (width / 2 + 1);
    Image.byte_h = height;    
...
{
        /* 7 colours are only applicable with 7in3 e-Paper */
        /* 16 colours are used for dithering */
		Image.scale = scale;
		Image.byte_w = (Image.mem_w % 2 == 0)? (Image.mem_w / 2 ): (Image.mem_w / 2 + 1);;
		// printf("scale = %d, byte_w = %d\r\n", Image.scale, Image.byte_w);
...
void Gui_SetPixel(unsigned short int x, unsigned short int y, unsigned short int color)
{
    ...
    if(Image.scale == 2){
        uint32_t Addr = xx / 8 + yy * Image.byte_w;
        uint8_t Rdata = Image.img[Addr];
        if(color == BLACK)
            Image.img[Addr] = Rdata & ~(0x80 >> (xx % 8));
        else
            Image.img[Addr] = Rdata | (0x80 >> (xx % 8));
    }else if(Image.scale == 4){
        uint32_t Addr = xx / 4 + yy * Image.byte_w;
        color = color % 4;//Guaranteed color scale is 4  --- 0~3
       
        Rdata = Rdata & (~(0xC0 >> ((xx % 4)*2)));//Clear first, then set value
        Image.img[Addr] = Rdata | ((color << 6) >> ((xx % 4)*2));
    }else if(Image.scale == 7 || Image.scale == 16){
		uint32_t Addr = xx / 2  + yy * Image.byte_w;
		uint8_t Rdata = Image.img[Addr];
		Rdata = Rdata & (~(0xF0 >> ((xx % 2)*4)));//Clear first, then set value
		Image.img[Addr] = Rdata | ((color << 4) >> ((xx % 2)*4));
		// printf("Add =  %d ,data = %d\r\n",Addr,Rdata);
	}
...
void Gui_Draw_Point(unsigned short int x, unsigned short int y, unsigned short int color, DOT_PIXEL Dot_Pixel, DOT_STYLE Dot_Style)
{
    ...
    if (x > Image.w || y > Image.h) 
		{
        printf("Gui_Draw_Point Input exceeds the normal display range\r\n");
        return;
    }
    if (Dot_Style == AROUND) 
		{
...
parameter:
    ...
    Dot_Pixe : Line width
    style    : Solid and dotted lines
******************************************************************************/
void Gui_Draw_Line(unsigned short int x1, unsigned short int y1, unsigned short int x2, unsigned short int y2,
                    unsigned short int color, DOT_PIXEL Dot_Pixel, LINE_STYLE style )
{ 
    unsigned short int x = x1;
    unsigned short int y = y1;
...
******************************************************************************/
void Gui_Draw_Rectangle(unsigned short int x1, unsigned short int y1, unsigned short int x2, unsigned short int y2,
                         unsigned short int color, DRAW_FILL filled, DOT_PIXEL Dot_Pixel)
{
...
parameter:
    x           :X coordinate
    y           :Y coordinate
    str_char       :To display the English characters
...
parameter:
    x           :x coordinate
    y           :y coordinate
    str_char          :The first address of the English string to be displayed
    Font             :A structure pointer that displays a character size
    Color_Foreground : Select the foreground color
    Color_Background : Select the background color
******************************************************************************/
void Gui_Draw_Str_EN(uint16_t x, uint16_t y, const char *str_char, FONT* Font, uint16_t Color_Background, uint16_t Color_Foreground )
{   uint16_t dx = x,dy = y;
    while(*str_char != '\0')
    {
                    ...
                    FONT* Font, uint16_t Color_Background, uint16_t Color_Foreground )
{
    ...
    //show
    Gui_Draw_Str_EN(x, y, (const char*)pStr, Font, Color_Background, Color_Foreground);
}
