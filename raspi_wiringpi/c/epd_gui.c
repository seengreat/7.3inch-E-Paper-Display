...
void Gui_SetScale(uint8_t scale)
{
    if(scale == 2){
        Image.scale = scale;
        Image.byte_w = (Image.mem_w % 8 == 0)? (Image.mem_w / 8 ): (Image.mem_w / 8 + 1);
    }else if(scale == 4){
        Image.scale = scale;
        Image.byte_w = (Image.mem_w % 4 == 0)? (Image.mem_w / 4 ): (Image.mem_w / 4 + 1);
...
//    }
    ...
    if(Image.scale == 2) {
		for (x = 0; y < Image.byte_h; y++) {
			for (x = 0; x < Image.byte_w; x++ ) {//8 pixel =  1 byte
				addr = x + y*Image.byte_w;
				Image.img[addr] = color;
			}
		}		
        ...
        for (y = 0; y < Image.byte_h; y++) {
			for (x = 0; x < Image.byte_w; x++ ) {
				addr = x + y*Image.byte_w;
				Image.img[addr] = (color<<6)|(color<<4)|(color<<2)|color;
			}
		}		
	}else if(Image.scale == 7 || Image.scale == 16) {
		for (y = 0; y < Image.byte_h; y++) {
